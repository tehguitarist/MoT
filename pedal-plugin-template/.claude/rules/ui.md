...
If you replace the procedurally-drawn pedal face with bitmap artwork, follow the checklist in
`ui/ui-replacements.md` (knobs authored at noon, consistent transparent buffer, 2x-for-100% asset
scale so the resizable UI stays sharp past 200%, crop instead of stretch, alpha + rotation-safety
for every non-texture image). It's easy to skip one of these and only notice the regression after
the UI-scale slider is exercised.
